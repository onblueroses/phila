...
5 fine-tune iterations, 750+ optimizer generations, 402 test scenarios across 3 independent suites.

**Jump to:** [Final Results](#final-results-summary) | [Research Notes](#research-notes) | [Experiment Log](#cycle-1--2026-04-01)

---

## Final Results Summary

### The journey: 15 days, 115 commits, 5 model versions

```
2026-03-24  Project created
2026-04-01  Cycle 1-3: multi-model eval, llama3.2 selected (94.1%)
2026-04-02  phila-ft v1: first fine-tune, buried-thread fixed (96.7%)
2026-04-04  phila-ft v2: regression fixes, speak-unanswered 83%->100% (95.8%)
2026-04-04  v3 architecture: hierarchical gate, dual-pass, cross-suite validation
2026-04-05  Overnight campaign: 4 configs x 2 rounds x 15 runs
2026-04-06  phila-ft v3: generalization gap closed, 93.3% independent (93.6%)
2026-04-07  phila-ft v5: targeted already-corrected augmentation (94.6% holdout)
2026-04-07  93-gen continuous optimizer: gen 46 best, plateau confirmed
2026-04-08  Prompt engineering: layer benchmark, restructure, double-check
2026-04-08  Final config: restructured prompt + phila-ft-v5 (95.2% holdout)
```

### Accuracy progression

| Version | Model | Holdout Acc | Independent Acc | Key improvement |
|---------|-------|------------|----------------|-----------------|
| Baseline | llama3.2 (stock) | 83.7% | 67.0% | - |
| v1 | phila-ft | 97.6% | - | Buried-thread 0%->100% |
| v2 | phila-ft-v2 | 93.0% | 76.7% | v1 regressions fixed |
| v3 | phila-ft-v3 | 93.6% | 93.3% | Generalization gap closed |
| v5 | phila-ft-v5 | 94.6% | - | Already-corrected improved |
| **v5+prompt** | **phila-ft-v5 + restructured** | **95.2%** | - | **False speaks 15->4, "already corrected" fixed** |

### What was built

- **5 fine-tuned models** (v1-v5), each targeting specific failure modes identified by the previous version's benchmark. QLoRA on RTX 4090, 3,799-4,780 training examples, zero holdout contamination.
- **146-scenario evaluation suite** with train/holdout split, bootstrap confidence intervals, confusion matrices. Started at 101, expanded to 140, then 146 as new failure categories emerged.
- **174-scenario independent suite** generated by Claude Opus from category definitions only - different author than the model, used to detect overfitting.
- **82-scenario adversarial suite** with sarcasm traps, phila-adjacent content, buried questions, code-switching.
- **Continuous optimizer** (750+ generations total) with mutation/crossover on system prompts, reward-hacking detection, holdout monitoring.
- **Layer benchmark** testing 5 prompt variants to find optimal example count.
- **4 architecture experiments**: monolithic, hierarchical (3 iterations), dual-pass (4 iterations). Monolithic won - 3B models can't decompose classification from reasoning.
- **Cross-suite validation** (4 configs x 3 rounds x 15 runs) proving generalization.

### Key discoveries

1. **Small models need examples, not descriptions.** Abstract instructions ("correct wrong facts") fail. Concrete worked examples ("person1: the great wall is in japan / correct response: speak") are what make 3B models work.

2. **Flat rules beat nested conditionals.** "Rule 2: correct wrong facts BUT if already corrected STAY SILENT" confused the model. Splitting into "NEVER SPEAK: already corrected" and "ALWAYS SPEAK: wrong fact uncorrected" fixed the longest-standing failure mode.

3. **Decomposition hurts at 3B scale.** Hierarchical gates, dual-pass architectures, and double-check passes all degraded accuracy. The monolithic single-pass gate works because classification and reasoning happen in one forward pass - splitting them loses the interconnection that small models rely on.

4. **7 examples is the sweet spot.** Below 7, false speaks are high (15). Above 7, the model goes too silent. The prompt length vs accuracy tradeoff has a clear optimum.

5. **Fine-tuning generalizes when training data matches the wild distribution.** v2 had a 13.6pp gap between hand-crafted and independent scenarios. v3 closed it to 0.3pp by training on Opus-generated examples from the same distribution the model sees in production.

6. **The generalization gap is the real metric.** Hand-crafted test suites inflate accuracy by ~15-20pp. The independent suite is the honest baseline. Building it was the single most important methodological decision.

7. **Recall is the bottleneck, not precision.** Across every config and every model version, precision stayed 0.95-1.00. The model almost never false-speaks. All improvement came from catching more speak-worthy moments without breaking the silence discipline.

### Final production configuration

```
Model:       phila-ft-v5 (llama3.2 3B, QLoRA, q4_k_m)
Gate:        monolithic single-pass (gate.ts)
Prompt:      NEVER SPEAK first, 7 worked examples, flat rules
Inference:   temperature=0.1, topP=0.52, numPredict=64
Latency:     832ms p50, 1120ms avg, 2498ms p95
Accuracy:    95.2% holdout [90.1%, 99.0% CI]
Precision:   0.983 (near-zero false speaks)
Recall:      0.912
F1:          0.946
```

### From stock to final: +11.5pp holdout, +26.3pp independent

The stock llama3.2 started at 83.7% holdout accuracy and 67.0% on independent scenarios. The final configuration reaches 95.2% holdout - a gain that came from five rounds of fine-tuning, each driven by failure analysis of the previous, plus systematic prompt engineering informed by research into how small models process instructions. The independent suite accuracy went from 67.0% to 93.3% (v3 measurement) - a +26.3pp improvement that proves the gains are real, not test-suite overfitting.
...
## v5 prompt engineering campaign (2026-04-07 - 2026-04-08)

### Context

After finetuning phila-ft-v5, ran a 93-generation continuous optimizer on the system prompt. Gen 46 was the best (extra-examples mutation), with 47 consecutive reverts after. Holdout flat at ~91-93% - model's ceiling for the original prompt format.

Extracted gen 46's winning additions and ran a systematic prompt engineering campaign to push past the ceiling.

### Layer benchmark (example count vs accuracy)

Tested 5 prompt variants with progressively more worked examples (3/5/7/9/11) to find the sweet spot. Run on VPS with phila-ft-v5, 3 runs per scenario.

| Layer | Examples | Prompt chars | Holdout Acc | 95% CI | False Speak | False Silent |
|-------|----------|-------------|-------------|--------|-------------|-------------|
| 0 | 3 | 853 | 94.9% | [89.9%, 98.7%] | 15 | 7 |
| 1 | 5 | 1191 | 93.2% | [87.3%, 98.3%] | 11 | 9 |
| 2 | 7 | 1649 | 94.9% | [89.9%, 98.7%] | 3 | 10 |
| 3 | 9 | 2171 | 94.9% | [89.9%, 98.7%] | 3 | 10 |
| 4 | 11 | 2681 | 94.1% | [88.6%, 98.7%] | 2 | 13 |

**Finding**: Layer 2 (7 examples) is the sweet spot. Same holdout as baseline but false speaks drop from 15 to 3. More examples (Layer 4, 11 ex) degrade holdout as the model starts going too silent (false silents rise from 7 to 13).

**Baked into production**: Layer 2's examples (already corrected + rhetorical question) added to gate.ts.

### Prompt restructure: NEVER SPEAK first (2026-04-08)

Research into system prompt patterns from major AI labs found that small models handle flat rules better than nested conditionals. The original prompt had "already corrected" as a nested exception inside rule 2 ("correct wrong facts BUT if someone already corrected..."). Restructured to:

```
NEVER SPEAK (check these first):
- someone already corrected the error -> STAY SILENT
...
1. direct address ("phila")
2. wrong fact uncorrected
3. unanswered factual question
```

**Results** (5 runs, 145 scenarios, commit 2bb1782):

| Metric | Baseline (Layer 2) | Restructured |
|--------|-------------------|--------------|
| Overall accuracy | 96.6% | 96.3% |
| Holdout accuracy | 94.9% | 95.2% |
| Holdout 95% CI | [89.9%, 98.7%] | [90.1%, 99.0%] |
| False speaks | 3 | 4 |
| False silents | 10 | 23 |
| Precision | - | 0.983 |
| Recall | - | 0.912 |
...
**Key observations**:
- Holdout improved slightly (94.9% -> 95.2%), CI tightened
- `already corrected partial` went from flaky to 100% - the flat rule structure fixed the nested conditional anti-pattern
- False silents increased (10 -> 23), mostly from recall-type scenarios (birthday recall 0%, checkout time 0%, who has tickets 0%, did anyone rsvp 0%) - the model defaults to silence when conversational questions require extracting info from earlier in the chat
- `phila cream cheese` regressed to 20% (1/5) - near-miss name handling got worse
- All "already corrected" variants: 100% (was the known limitation in v3/v4)

**Per-scenario failures** (0% or partial):
- `what day was the party`: 2/5 (40%) - recall
- `did anyone rsvp`: 0/5 - recall
- `what time is checkout`: 0/5 - recall
- `who has the tickets`: 0/5 - recall
- `birthday recall`: 0/5 - recall
- `phila cream cheese`: 1/5 (20%) - near-miss name

### Double-check pass (2026-04-08)

Added conditional second inference pass: when gate says "speak", a focused verification prompt checks for already-corrected errors, near-miss names, and rhetorical questions. Silent decisions (95% of traffic) stay single-pass.

**Results** (5 runs, 145 scenarios, commit d080ca6):

| Metric | Restructured (no double-check) | Restructured + double-check |
|--------|-------------------------------|----------------------------|
| Overall accuracy | 96.3% | 68.3% |
| Holdout accuracy | 95.2% | 62.5% |
| Precision | 0.983 | 1.000 |
| Recall | 0.912 | 0.115 |
| F1 | 0.946 | 0.207 |
| False speaks | 4 | 0 |
| False silents | 23 | 230 |
| Avg latency | 1120ms | 5103ms |

The double-check pass is catastrophically over-cautious. It achieves perfect precision (zero false speaks) but kills recall to 11.5% - overriding 230 out of 260 legitimate speak decisions to silent. The verification prompt is too aggressive at second-guessing. Not viable without a much more permissive verification prompt, and the latency cost (5x) makes it impractical regardless.

**Verdict**: Double-check rejected. Single-pass restructured prompt is the final production configuration.
