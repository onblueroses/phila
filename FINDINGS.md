...
### Results (2 rounds x 15 runs = 30 evaluations per config, round-to-round variance < 0.5pp)

| Config | Original Acc | Original F1 | Independent Acc | Independent F1 | Gap |
|--------|-------------|-------------|-----------------|----------------|-----|
| Mono base | 86.3% | 0.775 | 67.0% | 0.613 | -19.3pp |
| Mono ft-v2 | 90.3% | 0.850 | 76.7% | 0.756 | -13.6pp |
| Dual base | 87.0% | 0.800 | 67.1% | 0.613 | -19.9pp |
| **Dual ft-v2** | **91.9%** | **0.880** | **80.8%** | **0.806** | **-11.1pp** |
...
### Per-config precision/recall breakdown (15-run averages)
...
| Mono ft-v2 | Original | 1.000 | 0.740 | 1.000 | 0.000 |
| Mono ft-v2 | Independent | 0.984 | 0.613 | 0.986 | 0.014 |
| Dual base | Original | 0.947 | 0.692 | 0.977 | 0.023 |
| Dual base | Independent | 0.961 | 0.450 | 0.974 | 0.026 |
| Dual ft-v2 | Original | 0.977 | 0.801 | 0.989 | 0.011 |
| Dual ft-v2 | Independent | 0.986 | 0.682 | 0.986 | 0.014 |

### Next: dual-optimized fine-tuning dataset

The current fine-tune (phila-ft-v2, 1,138 examples) was trained for the monolithic gate. A v3 fine-tune should target the dual-pass architecture specifically:
- More memory-grounded examples (currently the weakest category)
- Examples from the independent scenario distribution (more diverse than hand-crafted)
- Training data for the extraction pipeline (what to extract vs ignore)
- Training data for Pass 2 memory-recall (when facts answer the question vs don't)
