...
The gate runs in two passes. Pass 1 is the proven monolithic gate - it handles factual corrections, unanswered questions, and direct address. Pass 2 kicks in only when Pass 1 says silent, checking whether someone is asking about something discussed earlier in the conversation ("where are we meeting?", "who said they'd bring drinks?"). Facts are extracted from each message batch and stored in SQLite for later recall.
...
| Fine-tune training examples | 1,138 v2 (v3 in progress: 15-20K across 3 task categories) |
| Fine-tune GPU | Vast.ai RTX 4090, QLoRA r=16 |
...
### cross-suite validation (15 runs x 2 rounds per config)

| configuration | original suite | independent suite | generalization gap |
|---------------|---------------|-------------------|-------------------|
| monolithic (base) | 86.3% | 67.0% | -19.3pp |
| monolithic (fine-tuned) | 90.3% | 76.7% | -13.6pp |
| dual-pass (base) | 87.0% | 67.1% | -19.9pp |
| **dual-pass (fine-tuned)** | **91.9%** | **80.8%** | **-11.1pp** |

The independent suite uses 174 scenarios generated by a different model (Claude Opus) from category definitions alone - no examples from the original test set. The 11-20pp gap between suites measures how well each configuration generalizes to unseen conversation patterns. The dual-pass fine-tuned configuration has the smallest gap.
