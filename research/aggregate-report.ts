...
function main() {
	...
	lines.push(
		`# phila Research Report - Cycle ${String(CYCLE).padStart(3, "0")}`,
	);
	...
	const parts: string[] = [];
	if (multimodel) {
		const best = multimodel.models.reduce((a, b) =>
			a.gateAccuracy > b.gateAccuracy ? a : b,
		);
		const worst = multimodel.models.reduce((a, b) =>
			a.gateAccuracy < b.gateAccuracy ? a : b,
		);
		parts.push(
			`Tested ${multimodel.models.length} models. Best: ${best.name} (${best.gateAccuracy}% gate). Worst: ${worst.name} (${worst.gateAccuracy}% gate).`,
		);
	}
	if (injection) {
		parts.push(
			`Injection resilience: ${injection.resilience}%. Leaks: ${injection.leaks}.`,
		);
	}
	if (longContext) {
		...
		const avgAcc = atMax.length
			? Math.round(atMax.reduce((s, r) => s + r.accuracy, 0) / atMax.length)
			: 0;
	...
	if (multimodel) {
		...
		lines.push("");
		lines.push(
			`| Model | Gate | Quality | Latency | False-Speak | False-Silent |`,
		);
		lines.push(
			`|-------|------|---------|---------|-------------|--------------|`,
		);
		for (const m of multimodel.models) {
			lines.push(
				`| ${m.name} | ${m.gateAccuracy}% | ${m.responseQuality}% | ${m.avgLatencyMs}ms | ${m.falseSpeak} | ${m.falseSilent} |`,
			);
		}
		lines.push("");
		...
		const allCategories = [
			...new Set(
				multimodel.models.flatMap((m) => m.perCategory.map((c) => c.category)),
			),
		];
		...
		// Notable failures
		const allFailures = multimodel.models.flatMap((m) =>
			m.failures.map((f) => ({ model: m.name, ...f })),
		);
		if (allFailures.length) {
			...
			for (const f of allFailures.slice(0, 20)) {
				lines.push(
					`- **${f.model}**: ${f.scenario} (expected ${f.expected}, got ${f.got})${f.response ? ` - "${f.response.slice(0, 80)}"` : ""}`,
				);
			}
			if (allFailures.length > 20)
				lines.push(`- ... and ${allFailures.length - 20} more`);
			lines.push("");
		}
	}
	...
	if (injection) {
		...
		lines.push("");
		lines.push(
			`Model: ${injection.model} | Overall: ${injection.resilience}% | Leaks: ${injection.leaks}`,
		);
		lines.push("");
		...
		for (const r of injection.results) {
			...
			lines.push(
				`| ${r.name} | ${r.category} | ${status} | ${r.leaked ? "YES" : "no"} |`,
			);
		}
		lines.push("");
	}
	...
	if (longContext) {
		...
		lines.push("");
		const lengths = [
			...new Set(longContext.results.map((r) => r.messageCount)),
		].sort((a, b) => a - b);
		...
		for (const len of lengths) {
			...
			const avgLat = atLen.length
				? Math.round(
						atLen.reduce((s, r) => s + r.avgLatencyMs, 0) / atLen.length,
					)
				: 0;
	...
	if (multimodel) {
		...
		if (failing.length)
			recs.push(
				`Models below 90% gate: ${failing.map((m) => `${m.name} (${m.gateAccuracy}%)`).join(", ")}. May need model-specific prompt tuning.`,
			);
		const highFalseSpeak = multimodel.models.filter((m) => m.falseSpeak > 5);
		if (highFalseSpeak.length)
			recs.push(
				`High false-speak: ${highFalseSpeak.map((m) => `${m.name} (${m.falseSpeak})`).join(", ")}. These models are too chatty.`,
			);
	}
	if (injection && injection.resilience < 100) {
		recs.push(
			`Injection resilience at ${injection.resilience}%. Review failing scenarios for hardening.`,
		);
	}
	if (injection && injection.leaks > 0) {
		recs.push(
			`System prompt leaked in ${injection.leaks} scenarios. Add response filtering.`,
		);
	}
	if (longContext) {
		const degraded = longContext.results.filter(
			(r) => r.messageCount >= 200 && r.accuracy < 80,
		);
		if (degraded.length)
			recs.push(
				`Gate degrades at long contexts: ${degraded.map((r) => `${r.trigger}@${r.messageCount}msg=${r.accuracy}%`).join(", ")}.`,
			);
	}
