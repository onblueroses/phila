...
async function infer(
	...
	return {
		content: ((await res.json()) as { message: { content: string } }).message
			.content,
		latencyMs,
	};
}
...
async function benchmarkModel(
	...
	for (const scenario of scenarios) {
		...
		for (let r = 0; r < runs; r++) {
			cat.total++;
			try {
				const { content, latencyMs } = await infer(
					system,
				...
				failures.push({
					...
					got: `error: ${e instanceof Error ? e.message : e}`,
				});
			}
		}
	...
	const perCategory: CategoryResult[] = [...categoryMap.entries()].map(
	...
	return {
		name: model,
		gateAccuracy: totalRuns
			? Math.round((totalCorrect / totalRuns) * 1000) / 10
			: 0,
		responseQuality: qualityCount
			? Math.round((qualitySum / qualityCount) * 1000) / 10
			: 0,
...
async function main() {
	...
	for (const model of models) {
		...
		results.push(result);
		console.log(
			`  gate: ${result.gateAccuracy}% | quality: ${result.responseQuality}% | latency: ${result.avgLatencyMs}ms`,
		);
		console.log(
			`  errors: ${result.falseSpeak} false-speak, ${result.falseSilent} false-silent`,
		);
		console.log();
	}
	...
	console.log(
		`  ${"model".padEnd(20)} ${"gate".padEnd(8)} ${"quality".padEnd(10)} ${"latency".padEnd(10)} ${"f-speak".padEnd(8)} f-silent`,
	);
	for (const r of results) {
		console.log(
			`  ${r.name.padEnd(20)} ${(`${r.gateAccuracy}%`).padEnd(8)} ${(`${r.responseQuality}%`).padEnd(10)} ${(`${r.avgLatencyMs}ms`).padEnd(10)} ${String(r.falseSpeak).padEnd(8)} ${r.falseSilent}`,
		);
	}

	// Write JSON output
	const outPath =
		args.out ?? `test/research-reports/multimodel-${Date.now()}.json`;
	writeFileSync(
		outPath,
		JSON.stringify(
			{ timestamp: new Date().toISOString(), models: results },
			null,
			2,
		),
	);
