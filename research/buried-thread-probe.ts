...
function generateScenarios(
	count: number,
): Array<{ conversation: string; topic: string }> {
	console.log(
		`Generating ${count} buried-thread scenarios via claude --print...`,
	);
...
for (const model of MODELS) {
	for (const [variantName, prompt] of Object.entries(PROMPT_VARIANTS)) {
		...
		process.stdout.write(
			`${(passRate * 100).toFixed(0)}% pass (avg score ${(totalScore / scenarios.length).toFixed(3)})\n`,
		);
	}
}
...
for (const model of MODELS) {
	const cells = variantNames.map((v) => {
		...
		const passRate =
			allResults.reduce((sum, r) => sum + r.passes, 0) / allResults.length;
...
for (const model of MODELS) {
	const cells = variantNames.map((v) => {
		...
		const avgScore =
			allResults.reduce((sum, r) => sum + r.avgScore, 0) / allResults.length;
...
lines.push(
	`**Best:** ${bestModel} + ${bestVariant} = ${(bestPass * 100).toFixed(1)}% pass rate`,
);
lines.push(``);
...
const baselinePass =
	scenarioResults
		.flatMap((s) =>
			s.results.filter(
				(r) => r.model === "llama3.2" && r.variant === "baseline",
			),
		)
		.reduce((sum, r) => sum + r.passes, 0) / scenarios.length;
lines.push(
	`**Baseline (llama3.2 + baseline prompt):** ${(baselinePass * 100).toFixed(1)}% pass rate`,
);
lines.push(``);
lines.push(
