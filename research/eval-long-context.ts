...
const TRIGGERS: TriggerTest[] = [
	...
	{
		name: "direct-address",
		trigger: "person1: hey phila whats the capital of france",
		expect: "speak",
	},
	{
		name: "unanswered-question",
		trigger: "person1: whats the tallest building in the world\nperson2: idk",
		expect: "speak",
	},
	{
		name: "no-trigger",
		trigger: "person1: anyway see you all later\nperson2: bye\nperson3: later",
		expect: "silent",
	},
];
...
async function main() {
	...
	console.log(
		`model: ${MODEL} | lengths: ${CONTEXT_LENGTHS.join(", ")} messages | runs: ${RUNS}`,
	);
	...
	// Warm-up
	try {
		await infer(system, "person1: test");
	} catch {
		/* ignore */
	}
		...
		for (const trigger of TRIGGERS) {
			...
			for (let r = 0; r < RUNS; r++) {
				try {
					...
					if (
						(trigger.expect === "silent" && isSilent) ||
						(trigger.expect === "speak" && !isSilent)
					) {
						passes++;
					}
				} catch {
			...
			const status =
				accuracy === 100 ? "PASS" : accuracy === 0 ? "FAIL" : `${accuracy}%`;
			console.log(
				`  ${trigger.name.padEnd(22)} ${status.padEnd(8)} ${avgLatency}ms`,
			);
		}
		console.log();
	}
	...
	console.log(
		`  ${"length".padEnd(10)} ${"wrong-fact".padEnd(12)} ${"direct".padEnd(12)} ${"unanswered".padEnd(12)} ${"silent".padEnd(12)} ${"avg-latency"}`,
	);
	for (const length of CONTEXT_LENGTHS) {
		...
		const avgLat = atLength.length
			? Math.round(
					atLength.reduce((s, r) => s + r.avgLatencyMs, 0) / atLength.length,
				)
			: 0;
		console.log(
			`  ${String(length).padEnd(10)} ${cols.map((c) => c.padEnd(12)).join("")} ${avgLat}ms`,
		);
	}

	const outPath =
		args.out ?? `test/research-reports/long-context-${Date.now()}.json`;
	writeFileSync(
		outPath,
		JSON.stringify(
			{
