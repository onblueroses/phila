...
async function runScenarios(
	...
	for (const scenario of scenarios) {
		...
		console.log(
			`  [${pass ? "PASS" : "FAIL"}] ${scenario.name} (expected ${scenario.expect}, got ${actual})`,
		);
	}
...
if (!values.out) {
	console.error(
		"Usage: node --experimental-strip-types research/gen-adversarial.ts --out <path> [--count N] [--findings <path>]",
	);
	process.exit(1);
}
...
console.log(
	`Validated ${scenarios.length} scenarios (${count - scenarios.length} discarded)`,
);
