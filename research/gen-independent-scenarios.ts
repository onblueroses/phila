...
const TARGET_COUNT = parseInt(args.count!)
...
const CATEGORIES = [
  {
    ...
    description: 'casual group chat: small talk, emotions, opinions, celebrations, jokes, banter, gossip, venting, compliments, dating talk, workout chat, food debates, weekend recaps, goodnight messages',
    count: 40,
  ...
  },
  {
    ...
    expect: 'silent' as const,
    description: 'someone states a wrong fact BUT another person already corrected them. the correction uses words like "actually", "no its", "thats not right", "pretty sure thats wrong"',
    count: 15,
  ...
  },
  {
    ...
    expect: 'silent' as const,
    description: 'rhetorical questions, hypotheticals, self-answered questions, existential musings, "would you rather" games',
    count: 10,
  ...
  },
  {
    ...
    expect: 'silent' as const,
    description: 'planning and coordination that is already handled: rides sorted, bills split, times agreed, tasks assigned and acknowledged',
    count: 10,
    difficulties: ['easy', 'medium'],
  },
  {
    ...
    description: 'someone states a WRONG fact (wrong date, wrong name, wrong number, wrong science, wrong geography, wrong history) and NOBODY corrects them. the agent should correct the error.',
    count: 25,
    difficulties: ['easy', 'medium', 'hard'],
  },
  {
    ...
    expect: 'speak' as const,
    description: 'someone asks a factual question and nobody answers correctly. others say "idk", give wrong answers, or change the subject. the agent should answer.',
    count: 20,
    difficulties: ['easy', 'medium', 'hard'],
  },
  {
    ...
    expect: 'speak' as const,
    description: 'someone addresses "phila" by name - greeting, question, request. phila should respond.',
    count: 15,
    difficulties: ['easy', 'medium'],
  },
  {
    ...
    expect: 'speak' as const,
    description: 'someone asks about plans/logistics discussed EARLIER in the conversation. e.g. "where are we meeting?", "what time again?", "which restaurant?" - the answer was stated by someone earlier in the chat.',
    count: 20,
    difficulties: ['medium', 'hard'],
  },
  {
    ...
    expect: 'speak' as const,
    description: 'someone asks about WHO committed to doing WHAT, discussed earlier. e.g. "who said theyd bring drinks?", "whos driving?", "did anyone say theyd pick up the cake?"',
    count: 15,
    difficulties: ['medium', 'hard'],
  },
  {
    ...
    expect: 'speak' as const,
    description: 'someone asks about a personal fact stated earlier in conversation. e.g. allergies, dietary restrictions, preferences, birthdays that were mentioned.',
    count: 10,
    difficulties: ['hard'],
  },
  {
    ...
    expect: 'silent' as const,
    description: 'edge cases that look like they should trigger a response but should NOT: sarcastic wrong facts, jokes with false claims, "philo"/"philadelphia"/"philanthropy" (not "phila"), questions about opinions not facts, hypothetical wrong facts, wrong facts that someone already corrected',
    count: 20,
...
async function generate(category: typeof CATEGORIES[number], difficulty: string): Promise<GeneratedScenario | null> {
...
${category.category.startsWith('speak-memory') ? 'IMPORTANT: the relevant fact/plan/commitment MUST appear earlier in the conversation, then someone asks about it later.' : ''}`
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { name?: string; conversation?: string; topic?: string }
...
async function main() {
  ...
  console.log(`generating ${TARGET_COUNT} independent scenarios using ${args.model}`)
    ...
    for (const diff of cat.difficulties) {
      ...
      while (generated < perDifficulty && attempts < maxAttempts) {
        ...
        if (scenario) {
          ...
          generated++
          process.stdout.write(`  [${scenarios.length}/${TARGET_COUNT}] ${cat.category}/${diff}: ${scenario.name}\n`)
        } else {
          process.stdout.write(`  [attempt ${attempts}] ${cat.category}/${diff}: generation failed, retrying\n`)
        }
      }
    }
  }
