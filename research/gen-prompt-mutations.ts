...
function buildFailuresBlock(failuresPath: string | undefined): string {
	...
	if (!existsSync(failuresPath)) {
		console.warn(
			`--failures file not found: ${failuresPath} (continuing without failure data)`,
		);
		return "";
	}
	...
	try {
		...
		) as AdversarialOutput;
	} catch (e) {
		console.warn(
			"Could not parse --failures file:",
	...
	const lines = failures.map(
		(f) => `  - "${f.name}": expected ${f.expect}, got ${f.actual}`,
	);
...
function parseMutations(raw: string): PromptMutation[] {
	...
	try {
		...
		console.warn(
			"JSON parse failed on claude output:",
			e instanceof Error ? e.message : e,
		);
		return [];
	}
...
function validateMutation(
	...
	| { valid: true; mutation: PromptMutation }
	| { valid: false; reason: string; name: string } {
	...
	if (typeof m.name !== "string")
		return { valid: false, reason: "name is not a string", name };
	if (typeof m.description !== "string")
		return { valid: false, reason: "description is not a string", name };
	if (typeof m.fullPrompt !== "string")
		return { valid: false, reason: "fullPrompt is not a string", name };
	...
	if (!fp.toLowerCase().includes("phila")) {
		return {
			valid: false,
			reason: 'fullPrompt does not contain "phila"',
			name,
		};
	}
	...
	if (!fp.toLowerCase().includes("json")) {
		return {
			valid: false,
			reason: "fullPrompt does not contain JSON output instruction",
			name,
		};
	}
	...
	if (fp.length < 200 || fp.length > 5000) {
		return {
			valid: false,
			reason: `fullPrompt length ${fp.length} outside 200-5000 range`,
			name,
		};
	}
	...
	if (fp === basePrompt) {
		return {
			valid: false,
			reason: "fullPrompt is identical to base prompt",
			name,
		};
	}
...
const claudeOutput = callClaude(
	buildClaudePrompt(count, basePrompt, failuresBlock),
);
