...
const { values } = parseArgs({
	options: {
		models: {
			type: "string",
			default: "llama3.2,qwen2.5:3b,qwen2.5:7b,gemma2:2b,phi3:mini",
		},
...
const outPath =
	values.out ?? `test/research-reports/model-compare-${Date.now()}.md`;
...
console.log(
	`Train: ${train.length} scenarios, Holdout: ${holdout.length} scenarios`,
);
...
const sorted = [...results].sort(
	(a, b) => b.train.compositeScore - a.train.compositeScore,
);
...
for (const r of sorted) {
	...
	const silentAcc =
		r.train.correctSilent /
		Math.max(1, r.train.correctSilent + r.train.falseSpeak);
	const speakAcc =
		r.train.correctSpeak /
		Math.max(1, r.train.correctSpeak + r.train.falseSilent);
	lines.push(
...
lines.push(``);
lines.push(
	`**Best train:** ${baseline.model} (${fmt(baseline.train.compositeScore)})`,
);
const bestHoldout = [...results].sort(
	(a, b) => b.holdout.compositeScore - a.holdout.compositeScore,
)[0];
lines.push(
	`**Best holdout:** ${bestHoldout.model} (${fmt(bestHoldout.holdout.compositeScore)})`,
);
lines.push(``);
