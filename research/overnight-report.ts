...
const tournament = tournamentPath
	? loadJson<TournamentOutput>(tournamentPath)
	: undefined;
const adversarial = adversarialPath
	? loadJson<AdversarialOutput>(adversarialPath)
	: undefined;
...
if (hacking) {
	...
	if (trainDelta <= 0.01)
		reasons.push(`train improvement marginal (${scoreFmt(trainDelta)})`);
	if (holdoutDelta <= 0)
		reasons.push(`holdout did not improve (${scoreFmt(holdoutDelta)})`);
...
if (tournament.tournament.length > 0) {
	const rows = tournament.tournament
		.map(
			(e) =>
				`| ${e.name} | ${scoreFmt(e.trainScore)} | ${e.tTestP.toFixed(4)} | ${e.accepted ? "accepted" : "rejected"} |`,
		)
		.join("\n");
...
if (tournament.qualityDive && tournament.qualityDive.length > 0) {
	const rows = tournament.qualityDive
		.map(
			(d) =>
				`| ${d.name} | ${scoreFmt(d.mean)} | ${scoreFmt(d.stddev)} | ${scoreFmt(d.min)} | ${scoreFmt(d.max)} |`,
		)
		.join("\n");
...
const outPath =
	values.out ?? `test/research-reports/overnight-round-${roundLabel}.md`;
