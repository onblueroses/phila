...
function buildConversation(row: Row): string {
	...
	for (const turn of row.context_turns) {
		const label =
			turn.speaker === row.target_speaker
				? "you"
				: speakerLabel(turn.speaker, speakers);
		lines.push(`${label}: ${turn.text.trim()}`);
	}

	const currentLabel =
		row.current_turn.speaker === row.target_speaker
			? "you"
...
function buildAssistantResponse(row: Row): string {
	if (row.decision === "SPEAK") {
		return JSON.stringify({
			action: "speak",
			reason: row.reason || "relevant context",
			response: "(training placeholder)",
		});
	}
...
async function main() {
	...
	const response = await fetch(
		"https://huggingface.co/api/datasets/ishiki-labs/multi-party-dialogue",
	);
	...
	const dataUrl =
		"https://huggingface.co/datasets/ishiki-labs/multi-party-dialogue/resolve/main/data/train-00000-of-00001.jsonl";
