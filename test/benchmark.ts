...
function loadScenarios() {
	if (args.scenarios) {
		...
		const external = JSON.parse(raw) as Array<{
			name: string;
			conversation: string;
			expect: string;
			category?: string;
		...
		console.log(
			`loaded ${external.length} external scenarios from ${args.scenarios}`,
		);
...
function conversationToMessages(conversation: string): ChatMessage[] {
	return conversation.split("\n").map((line, i) => {
		...
		const sender =
			colonIdx > 0 ? line.slice(0, colonIdx).trim() : `person${i + 1}`;
...
async function runBenchmark(config: RunConfig): Promise<ScenarioResult[]> {
	...
	for (const scenario of SCENARIOS) {
		...
		for (let i = 0; i < config.runs; i++) {
			try {
				const { content, latencyMs } = await inferTimed(
					system,
...
async function runHierarchicalBenchmark(
	config: RunConfig,
): Promise<ScenarioResult[]> {
	const profile: GroupProfile = {
		chatId: "bench",
	...
	for (const scenario of SCENARIOS) {
		...
		for (let i = 0; i < config.runs; i++) {
			try {
				const start = performance.now();
				const decision = await evaluateHierarchical(
					messages,
					profile,
					philaConfig,
					ctx,
				...
				if (decision.action === scenario.expect) {
					...
					if (i === 0) {
						process.stdout.write(
							`    FAIL: expected=${scenario.expect} got=${decision.action} stages=${decision.stages.join("->")}\n`,
						);
					}
				}
			} catch (err) {
...
async function runDualBenchmark(config: RunConfig): Promise<ScenarioResult[]> {
	...
	for (const [si, scenario] of SCENARIOS.entries()) {
		...
		const messages = conversationToMessages(scenario.conversation).map((m) => ({
			...m,
			chatId: scenarioChatId,
		}));
		const scenarioProfile: GroupProfile = {
			chatId: scenarioChatId,
			speakBias: 0.0,
			updatedAt: Date.now(),
		};
		...
		for (let i = 0; i < config.runs; i++) {
			try {
				const start = performance.now();
				const decision = await evaluateDual(
					messages,
					messages,
					scenarioProfile,
					philaConfig,
...
function summarize(results: ScenarioResult[], config: RunConfig) {
	...
	const accuracy = totalRuns
		? Math.round((totalPasses / totalRuns) * 1000) / 10
		: 0;
	const avgLat = allLatencies.length ? Math.round(avg(allLatencies)) : 0;
	const p50 = allLatencies.length
		? Math.round(percentile(allLatencies, 0.5))
		: 0;
	const p95 = allLatencies.length
		? Math.round(percentile(allLatencies, 0.95))
		: 0;
...
const PARAM_SWEEP: Omit<RunConfig, "model" | "ollamaUrl" | "runs" | "seed">[] =
	[
		...
		{
			label: "very-low-temp (t=0.1)",
			temperature: 0.1,
			numPredict: 256,
			topP: 0.9,
		},
		{
			label: "zero-temp (t=0.0)",
			temperature: 0.0,
			numPredict: 256,
			topP: 1.0,
		},
		{
			label: "high-temp (t=1.0)",
			temperature: 1.0,
			numPredict: 256,
			topP: 0.9,
		},
		{
			label: "short-output (np=64)",
			temperature: 0.7,
			numPredict: 64,
			topP: 0.9,
		},
		{
			label: "tight-topp (tp=0.5)",
			temperature: 0.7,
			numPredict: 256,
			topP: 0.5,
		},
		{
			label: "focused (t=0.1,tp=0.5)",
			temperature: 0.1,
			numPredict: 128,
			topP: 0.5,
		},
...
async function main() {
	...
	if (args.sweep) {
		...
		for (const params of PARAM_SWEEP) {
			...
			console.log(
				`  accuracy: ${s.accuracy}% (${s.totalPasses}/${s.totalRuns}) | latency: ${s.avgLat}ms avg, ${s.p50}ms p50, ${s.p95}ms p95\n`,
			);
		}

		console.log("=== sweep summary ===");
		console.log(
			`  ${pad("config", 28)} ${pad("accuracy", 10)} ${pad("avg ms", 10)} ${pad("p50 ms", 10)} p95 ms`,
		);
		for (const r of sweepResults) {
			console.log(
				`  ${pad(r.label, 28)} ${pad(`${r.accuracy}%`, 10)} ${pad(r.avgLat, 10)} ${pad(r.p50, 10)} ${r.p95}`,
			);
		}
		...
		for (const m of models) {
			...
			const s = summarize(results, config);
			console.log(
				`  accuracy: ${s.accuracy}% | latency: ${s.avgLat}ms avg, ${s.p50}ms p50\n`,
			);
		}
	} else {
		...
		const holdoutScores = holdoutResults.map(
			(r) => r.passes / (r.passes + r.fails + r.errors),
		);
		...
		console.log(
			`holdout accuracy: ${(ci.mean * 100).toFixed(1)}% [${(ci.lower * 100).toFixed(1)}%, ${(ci.upper * 100).toFixed(1)}%] (95% CI, 10000 bootstrap)`,
		);
