...
async function main() {
  ...
  if (cp) {
    ...
    const baseline = await evaluate(basePrompt, bestConfig, train, RUNS, BASE_URL)
    printResult(baseline)

    console.log('--- baseline (holdout) ---')
    const holdoutBaseline = await evaluate(basePrompt, bestConfig, holdout, RUNS, BASE_URL)
  ...
  while (running && (MAX_GENERATIONS === 0 || generation < MAX_GENERATIONS)) {
    ...
    try {
      const trainResult = await evaluate(trialPrompt, trialConfig, train, RUNS, BASE_URL)
      const holdoutResult = await evaluate(trialPrompt, trialConfig, holdout, RUNS, BASE_URL)
