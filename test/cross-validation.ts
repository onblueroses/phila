...
export function stratifiedKFold(scenarios: Scenario[], k: number): Scenario[][] {
...
// -- Evaluation (standalone, no Ollama dependency for import) --
...
async function infer(system: string, user: string, config: InferenceConfig, ollamaUrl: string): Promise<string> {
...
async function evaluateFold(
  ...
  runs: number,
  ollamaUrl: string,
): Promise<{ gateScore: number; compositeScore: number; perScenarioScores: Map<string, number[]> }> {
  let correctSilent = 0
  let correctSpeak = 0
  let falseSpeak = 0
  let falseSilent = 0
  ...
  for (const scenario of scenarios) {
    ...
    for (let r = 0; r < runs; r++) {
      ...
      try {
        const raw = await infer(systemPrompt, scenario.conversation, config, ollamaUrl)
...
export async function crossValidate(
  ...
  for (let i = 0; i < k; i++) {
    ...
    const result = await evaluateFold(systemPrompt, config, valFold, runs, ollamaUrl)
  ...
  const ci95: [number, number] = [mean - 1.96 * std / Math.sqrt(k), mean + 1.96 * std / Math.sqrt(k)]
  ...
  for (const [name, scores] of allPerScenario) {
    ...
    const sVar = scores.reduce((s, v) => s + (v - sMean) ** 2, 0) / (scores.length - 1)
    if (sVar > 0.25) {
...
async function main() {
  ...
  const config: InferenceConfig = { model: 'llama3.2', temperature: 0.1, numPredict: 64, topP: 0.52 }

  console.log(`=== phila cross-validation ===`)
  console.log(`k=${k} | runs=${runs} | scenarios=${scenarios.length} (train only)`)
  console.log()

  const result = await crossValidate(systemPrompt, config, scenarios, k, runs, ollamaUrl)

  for (const fold of result.foldResults) {
    console.log(`  fold ${fold.fold}: composite=${(fold.compositeScore * 100).toFixed(1)}% gate=${(fold.gateScore * 100).toFixed(1)}% (${fold.scenarioCount} scenarios)`)
  }
  ...
  if (result.flakyScenarios.length) {
    ...
    for (const f of result.flakyScenarios) {
      console.log(`  ${f.name}: variance=${f.variance.toFixed(2)} passRate=${(f.passRate * 100).toFixed(0)}%`)
    }
  }
}
