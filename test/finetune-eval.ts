...
const REGRESSION_RUNS = parseInt(args["regression-runs"]!, 10);
...
async function evalScenarios(
	...
	for (const scenario of scenarios) {
		...
		for (let i = 0; i < runs; i++) {
			const { decision, response, latencyMs } = await inferOne(
				system,
			...
			const expectedAction =
				scenario.expect === "silent" ? GateAction.SILENT : GateAction.SPEAK;
...
function summarize(summaries: ScenarioSummary[]) {
	...
	const avgQuality = speakOnes.length
		? speakOnes.reduce((n, s) => n + s.compositeSum / s.compositeCount, 0) /
			speakOnes.length
		: null;
	...
	const avgLat = Math.round(
		allLatencies.reduce((n, v) => n + v, 0) / allLatencies.length,
	);
...
console.log(
	`Ollama: ${OLLAMA_URL}  |  runs: ${RUNS}  |  regression runs: ${REGRESSION_RUNS}`,
);

// ── 1. Holdout-only accuracy ───────────────────────────────────────────────
printSection(
	`1. HOLDOUT ACCURACY (n=${holdoutScenarios().length} scenarios × ${RUNS} runs)`,
);
...
console.log(`\n[${baselineModel}]`);
const baseHoldout = await evalScenarios(
	holdoutScenarios(),
	baselineModel,
	RUNS,
);
...
printSection(
	`2. COMPOSITE SCORING — all ${SCENARIOS.length} scenarios × ${RUNS} runs`,
);
...
console.log("\nFull composite:");
console.log(
	`  ${baselineModel.padEnd(16)} gate=${(baseA.acc * 100).toFixed(1)}%  quality=${(baseA.avgQuality ?? 0).toFixed(3)}  composite=${baseA.composite.toFixed(4)}  lat=${baseA.avgLat}ms p50=${baseA.p50}ms`,
);
console.log(
	`  ${ftModel.padEnd(16)} gate=${(ftA.acc * 100).toFixed(1)}%  quality=${(ftA.avgQuality ?? 0).toFixed(3)}  composite=${ftA.composite.toFixed(4)}  lat=${ftA.avgLat}ms p50=${ftA.p50}ms`,
);
...
for (const [cat, base] of baseCats) {
	...
	console.log(
		`  ${cat.padEnd(30)} baseline=${basePct.padStart(3)}%  ft=${ftPct.padStart(3)}%${marker}`,
	);
}
...
const regressionSet = SCENARIOS.filter((s) =>
	REGRESSION_SCENARIOS.includes(s.name),
);
console.log(
...
console.log(`\n[${baselineModel}]`);
const baseReg = await evalScenarios(
	regressionSet,
	baselineModel,
	REGRESSION_RUNS,
);
...
for (const s of ftReg) {
	...
	console.log(
		`  ${marker} ${s.name}: baseline=${basePct}%  ft=${ftPct}%  (${REGRESSION_RUNS} runs)`,
	);
}
...
const report = {
	...
	regressions: ftReg.map((s, i) => ({
		...s,
		baselineGatePass: baseReg[i].gatePass,
		baselineGateTotal: baseReg[i].gateTotal,
	})),
};

const outPath =
	args.out ?? `test/research-reports/finetune-eval-${Date.now()}.json`;
