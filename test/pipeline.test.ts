...
describe("pipeline integration", () => {
	...
	it("speak decision flows through voice filter", () => {
		...
		const raw =
			'{"action":"speak","reason":"wrong fact","response":"The Eiffel Tower is in Paris, not London."}';
	...
	it("feedback adjusts profile used by gate prompt", () => {
		...
		assert.ok(
			!prompt.includes("strongly prefers"),
			"single negative should not trigger strongest tier yet",
		);

		// Two more negatives -> crosses -0.15 threshold
		mem.applyFeedback("c1", {
			type: "negative",
			context: "stop phila",
			timestamp: 2000,
		});
	...
	it("voice filter strips AI-speak from gate response", () => {
		const raw =
			'{"action":"speak","reason":"direct address","response":"Great question! I\'d be happy to help. The answer is 42."}';
		const decision = parseDecision(raw);
		assert.equal(decision.action, GateAction.SPEAK);
