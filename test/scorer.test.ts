...
function scenario(overrides: Partial<Scenario> = {}): Scenario {
	return {
		...
		category: "silent-social" as const,
...
describe("scoreTopicAccuracy", () => {
	it("passes when required keywords present and forbidden absent", () => {
		const s = scenario({
			validators: [{ required: ["paris"], forbidden: ["london"] }],
		});
	...
	it("fails when forbidden keyword present", () => {
		const s = scenario({
			validators: [{ required: ["correct"], forbidden: ["wrong"] }],
		});
...
describe("scoreCasualness", () => {
	...
	it("penalizes hedging", () => {
		const score = scoreCasualness(
			"I would suggest checking the documentation for details.",
		);
		assert.ok(score < 1.0);
	});
	...
	it("penalizes long sentences", () => {
		const score = scoreCasualness(
			"the answer to your inquiry regarding the tallest mountain in the entire world is actually mount everest which stands at about eight thousand meters",
		);
...
describe("scoreAiSpeakAbsence", () => {
	...
	it("deducts for meta patterns", () => {
		assert.ok(
			scoreAiSpeakAbsence("As an AI, I don't have personal opinions.") < 1.0,
		);
	});
	...
	it("stacks deductions for multiple matches", () => {
		const score = scoreAiSpeakAbsence(
			"Great question! I'd be happy to help. Certainly, the answer is 42.",
		);
		assert.ok(score <= 0.6);
	});
...
describe("scoreResponse composite", () => {
	it("weights sum to 1.0", () => {
		const s = scenario({
			topic: "paris",
			validators: [{ required: ["paris"], forbidden: [] }],
		});
		const breakdown = scoreResponse("paris", s);
		// All dimensions produce a value; composite is weighted sum
		const manual =
			breakdown.topicAccuracy * 0.35 +
			breakdown.casualness * 0.25 +
			breakdown.aiSpeakAbsence * 0.2 +
			breakdown.lengthFit * 0.1 +
			breakdown.voiceSurvival * 0.1;
	...
	it("good response scores high", () => {
		...
		const breakdown = scoreResponse(
			"the eiffel tower is in paris, not london at all",
			s,
		);
...
describe("compositeWeights", () => {
	...
	it("keeps default weights when gate < 99%", () => {
		...
		assert.equal(w.gate, 0.7);
		assert.equal(w.quality, 0.2);
		assert.equal(w.latency, 0.1);
	});
});
