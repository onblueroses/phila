...
export function scoreTopicAccuracy(response: string, scenario: Scenario): number {
...
const FORMAL_CONNECTORS = /\b(furthermore|moreover|additionally|consequently|nevertheless|in conclusion|as a result|it is important to note)\b/i
const HEDGING = /\b(i would suggest|it might be|perhaps you could|one could argue|it's worth noting|i should mention)\b/i
...
export function scoreCasualness(response: string): number {
  ...
  const titleCase = response.split(' ').every((w) => w[0] === w[0]?.toUpperCase()) && response.length > 10
...
export function scoreResponse(response: string, scenario: Scenario): ScoreBreakdown {
  ...
  return { topicAccuracy, casualness, aiSpeakAbsence, lengthFit, voiceSurvival, composite }
}

// -- Auto-rebalancing composite weights --

export function compositeWeights(gateAccuracy: number): { gate: number; quality: number; latency: number } {
  if (gateAccuracy >= 0.99) return { gate: 0.40, quality: 0.45, latency: 0.15 }
  return { gate: 0.70, quality: 0.20, latency: 0.10 }
}
